...
func _init(_manifest: ModManifest, path: String) -> void :
    ...
    if mod_loader_store:
        is_locked = manifest.get_mod_id() in mod_loader_store.ml_options.locked_mods
    else:
        is_locked = false
...
func set_mod_state(should_activate: bool, force: = false) -> bool:
    if is_locked and should_activate != is_active:
        var mod_loader_store = Engine.get_main_loop().root.get_node_or_null("ModLoaderStore")
        var locked = []
        if mod_loader_store: locked = mod_loader_store.ml_options.locked_mods
        ModLoaderLog.error(
            "Unable to toggle mod \"%s\" since it is marked as locked. Locked mods: %s"
            %[manifest.get_mod_id(), locked], LOG_NAME)
        return false
...
func _has_required_files() -> bool:
    ...
    for required_file in RequiredModFiles:
        ...
        if not _ModLoaderFile.file_exists(required_file_path, zip_path):
            var mod_loader_store = Engine.get_main_loop().root.get_node_or_null("ModLoaderStore")
            var url = mod_loader_store.URL_MOD_STRUCTURE_DOCS if mod_loader_store else "https://github.com/GodotModding/godot-mod-loader"
            load_errors.push_back(
                "ERROR - %s is missing a required file: %s. For more information, please visit \"%s\"." %
                [dir_name, required_file_path, url]
            )
