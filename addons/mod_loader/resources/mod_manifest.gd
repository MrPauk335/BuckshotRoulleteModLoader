...
func validate(manifest: Dictionary, path: String) -> bool:
    ...
    if mod_loader_store:
        if mod_loader_store.ml_options.game_version_validation == ModLoaderOptionsProfile.VERSION_VALIDATION.DEFAULT:
            _is_game_version_compatible(mod_id)

        if mod_loader_store.ml_options.game_version_validation == ModLoaderOptionsProfile.VERSION_VALIDATION.CUSTOM:
            if mod_loader_store.ml_options.custom_game_version_validation_callable:
                mod_loader_store.ml_options.custom_game_version_validation_callable.call(self)
            else:
