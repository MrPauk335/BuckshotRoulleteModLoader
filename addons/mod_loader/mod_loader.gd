...
func register_mods() -> void:
    ...
    for mod_path in mod_paths:
        ...
        if not mod.load_errors.is_empty():
            ...
            ModLoaderLog.error("The mod from path \"%s\" cannot be loaded. ModData initialization failed." % mod_path, LOG_NAME)

        if ModLoaderStore.mod_data.has(mod.dir_name):
            continue

        ModLoaderStore.mod_data[mod.dir_name] = mod

        if mod.is_loadable:
            if _ModLoaderPath.is_zip(mod_path):
                if not ProjectSettings.load_resource_pack(mod_path, false):
                    ModLoaderLog.error("Failed to load mod zip from path \"%s\"." % mod_path, LOG_NAME)
                    continue
    ...
    _init_configs_and_dependencies()


func _init_configs_and_dependencies() -> void:
    ...
    for dir_name in ModLoaderStore.mod_data:
        ...
        if not mod.is_loadable: continue
