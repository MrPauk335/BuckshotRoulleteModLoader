...
func ShowFinalTargetOnScreen(for_direction: String):
	...
	await get_tree().create_timer(0.64, false).timeout
	var am = 0
	while looping:
...
func BootupFinished():
	...
	if properties.cpu_enabled or (properties.user_id == GlobalSteam.STEAM_ID and GlobalVariables.mp_player_ai_enabled):

		var valid_indices = []
		for i in range(valid_target_array.size()):
			if valid_target_array[i]: valid_indices.append(i)


		if valid_indices.size() > 0:
			var rand_idx = valid_indices[randi() % valid_indices.size()]
			var direction = target_direction_strings[rand_idx]
			print("[BOT] Auto-selecting jammer target: ", direction)
			GetInput(direction)
			await get_tree().create_timer(0.4, false).timeout
			GetInput("confirm")
		else:
			print("[BOT] No valid jammer targets found. Trying to cancel...")
			GetInput("confirm")
