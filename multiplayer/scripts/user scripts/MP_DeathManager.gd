...
func UserDeath_FirstPerson():
    ...
    speaker_glimpse.pitch_scale = randf_range(0.8, 1)
...
func PlaySound_CorpseFall():
    ...
    speaker_corpse_fall.pitch_scale = randf_range(0.9, 1)
...
func _bot_check_retrigger_turn():
    if (properties.cpu_enabled and GlobalSteam.STEAM_ID == GlobalSteam.HOST_ID) or (properties.user_id == GlobalSteam.STEAM_ID and GlobalVariables.mp_player_ai_enabled):
        if properties.intermediary.game_state.MAIN_active_current_turn_socket == properties.socket_number:
            print("[BOT] Revival finished on turn, triggering AI logic.")
...
func DefibRevive():
    if properties.is_spectating:
        properties.viewblocker.FadeIn(0.7, -1.8)
    speaker_defib.play()
