...
func _bot_check_retrigger_turn():
	if (properties.cpu_enabled and GlobalSteam.STEAM_ID == GlobalSteam.HOST_ID) or (properties.user_id == GlobalSteam.STEAM_ID and GlobalVariables.mp_player_ai_enabled):
		if properties.intermediary.game_state.MAIN_active_current_turn_socket == properties.socket_number:
			print("[BOT] Item grabbing finished on turn, triggering AI logic.")
			properties.intermediary.roundManager.BeginBotTurn.call_deferred(properties)
...
func ReceivePacket_PlaceItem(dict: Dictionary):
	...
	if dict.socket_number == properties.socket_number:
		...
		if properties.is_active:
			...
			if dict.is_last_item:
				EndItemGrabbingDefault()
...
func GrabItem(item_id: int):
	...
	pickup_indicator.snapping_to_min = false
...
func PlayItemPullSound_FirstPerson():
	speaker_fp_grab_item.pitch_scale = randf_range(0.95, 1)
...
func PlayItemPullSound_ThirdPerson():
	speaker_tp_grab_item.pitch_scale = randf_range(0.95, 1)
...
func ForceClearAllItems():
	print("[ITEM] Force clearing all items for socket: ", properties.socket_number)
	if item_spawn_parent_local != null:
		for c in item_spawn_parent_local.get_children():
			if c.name != "pos_item pickup":
				c.queue_free()
	ClearInventory_Dictionaries()
	properties.is_holding_item_to_place = false
...
func _bot_auto_grab_items():
	var num_items = properties.intermediary.game_state.MAIN_active_num_of_items_to_grab


	var free_slots = 0
	for item in properties.user_inventory:
		if item == {}:
			free_slots += 1

	if free_slots < num_items:
		num_items = free_slots

	if num_items <= 0:
		print("[BOT] Inventory full for socket ", properties.socket_number, ". Finishing grab phase instantly.")
		EndItemGrabbingDefault()
		if GlobalSteam.STEAM_ID == GlobalSteam.HOST_ID:
			properties.intermediary.game_state.MAIN_active_num_of_users_finished_item_grabbing += 1
			properties.intermediary.game_state.CheckIfItemGrabbingFinishedForAllUsers()
	...
	print("[BOT] Auto-grabbing ", num_items, " items for socket ", properties.socket_number)
	for i in range(num_items):
		...
		properties.intermediary.packets.PipeData(grab_packet)



		var wait_ticks = 0
		while not properties.is_holding_item_to_place and wait_ticks < 50:
			await get_tree().create_timer(0.1, false).timeout
			wait_ticks += 1


		if not properties.is_holding_item_to_place:
			print("[BOT] Did not receive item from server (pool empty?). Aborting grab loop.")
			break


		var found_slot = -1
		for check_idx in range(properties.user_inventory.size()):
			if properties.user_inventory[check_idx] == {}:
				found_slot = check_idx
				break
		...
		properties.user_inventory[found_slot] = {"reserved": true}
		...
		var place_packet = {
			...
			"packet_id": 19, 
			"sent_from_socket": properties.socket_number, 
			"local_grid_index": found_slot, 
			"sender": "bot"
		}
		properties.intermediary.packets.PipeData(place_packet)
		await get_tree().create_timer(0.5, false).timeout
	print("[BOT] Finished auto-grabbing items for socket ", properties.socket_number)
