	...
	match alias:
		...
		"add bot":
			if GlobalSteam.STEAM_ID == GlobalSteam.HOST_ID:
				...
				if total < 4:
					GlobalVariables.mp_bot_count += 1
					Console_Copypaste("BOT ADDED (" + str(GlobalVariables.mp_bot_count) + " bot(s))")
					lobby_ui.UpdatePlayerList()
					print("[BOTS] Added bot. Total bots: ", GlobalVariables.mp_bot_count)
					BroadcastBotCount()
				else:
					Console_Copypaste("LOBBY FULL (max 4)")
		"remove bot":
			if GlobalSteam.STEAM_ID == GlobalSteam.HOST_ID:
				if GlobalVariables.mp_bot_count > 0:
					GlobalVariables.mp_bot_count -= 1
					Console_Copypaste("BOT REMOVED (" + str(GlobalVariables.mp_bot_count) + " bot(s))")
					lobby_ui.UpdatePlayerList()
					print("[BOTS] Removed bot. Total bots: ", GlobalVariables.mp_bot_count)
					BroadcastBotCount()
...
func leave_lobby() -> void :
	...
	GlobalVariables.mp_bot_count = 0
...
func BroadcastBotCount():
	if GlobalSteam.STEAM_ID != GlobalSteam.HOST_ID: return
	var packet = {
		"packet category": "MP_LobbyManager", 
		"packet alias": "sync bot count", 
		"sent_from": "host", 
		"packet_id": 34, 
		"bot_count": GlobalVariables.mp_bot_count, 
	}
	packets.send_p2p_packet(0, packet)
	if GlobalVariables.mp_debugging: packets.PipeData(packet)

func ReceivePacket_SyncBotCount(packet: Dictionary):
	if GlobalSteam.STEAM_ID == GlobalSteam.HOST_ID: return
	GlobalVariables.mp_bot_count = packet.bot_count
	lobby_ui.UpdatePlayerList()
	print("[BOTS] Synced bot count from host: ", GlobalVariables.mp_bot_count)
