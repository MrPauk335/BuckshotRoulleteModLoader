...
@export var socket_rotations: Array[Vector3]
...
func MoveToSocket(socket_next: float, direction: String = ""):
	...
	if (waiting_for_unlock): get_tree().create_timer(0.15, false).timeout
...
func PlayMovementSound():
	...
	speaker_rotation_hit.pitch_scale = randf_range(0.85, 1)
