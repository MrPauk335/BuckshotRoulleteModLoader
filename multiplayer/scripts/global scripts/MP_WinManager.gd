...
func WinRoutine(winning_socket_number: int = -1):
	...
	if GlobalVariables.mp_auto_battler_enabled:
		...
		for i in instance_handler.instance_property_array:
			if i.socket_number == winning_socket_number:
				winner_name = i.user_name
				break
		GlobalVariables.LogAnalytics(winner_name, roundManager.game_state.MAIN_active_round_index, winning_socket_number)


		await get_tree().create_timer(7.0, false).timeout
		print("[AUTO] Auto-Battler: Returning to lobby for next match...")
