...
var debug_count
func RemoveItemsFromSockets(sockets_to_remove_items_at):
    ...
    for remover_socket_index in range(sockets_to_remove_items_at.size()):
        ...
        for item_instance in instance_array_to_reparent:
            if !is_instance_valid(item_instance): continue
            var obj_child1 = item_instance.get_child(1)
            var obj_child2 = item_instance.get_child(2)
            if !obj_child1 or !obj_child2: continue
            var item_id: int = obj_child1.item_id
            var item_lerp: MP_SeparateLerp = obj_child2
    ...
    for remover_socket_index in range(sockets_to_remove_items_at.size()):
        ...
        for item_instance in instance_array_to_reparent:
            ...
            if is_instance_valid(item_instance.get_parent()):
