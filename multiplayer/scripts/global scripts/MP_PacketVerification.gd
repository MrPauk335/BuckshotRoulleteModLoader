...
func VerifyPacket(packet_data):
	if _ModLoaderHooks.any_mod_hooked:
		return _ModLoaderHooks.call_hooks(self.vanilla_VerifyPacket, [packet_data], _ModLoaderHooks.get_hook_hash("res://multiplayer/scripts/global scripts/MP_PacketVerification.gd", "VerifyPacket"))
...
func PacketSort(dict: Dictionary):
    ...
    match value_alias:
        "pickup shotgun request":
            game_state.MAIN_is_shotgun_held = true
            packet = {
        ...
        "shoot user request":
            if game_state.MAIN_active_sequence_dict.sequence_in_shotgun.size() == 0:
                print("Ignoring shoot request: shotgun is empty")
                return
