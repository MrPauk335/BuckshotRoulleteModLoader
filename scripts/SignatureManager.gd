...
func PickUpWaiver():
    ...
    blinking = true
    BlinkMarker()
    await get_tree().create_timer(2.77, false).timeout
...
func GetInput(letterAlias: String, specialAlias: String):
    speaker_keypress.pitch_scale = randf_range(0.95, 1)
...
func Input_Enter():
    ...
    for i in range(lettercount):
        ...
        speaker_punch.pitch_scale = randf_range(0.95, 1)
        speaker_punch.play()
