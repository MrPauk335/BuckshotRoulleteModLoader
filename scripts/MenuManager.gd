...
func _ready():
    ...
    for b in buttons:
        ...
        if b.alias == "mods":
            if b.is_connected("is_pressed", OnModsPressed): b.disconnect("is_pressed", OnModsPressed)
            b.connect("is_pressed", OnModsPressed)
        if b.alias == "exit":
    ...
    _setup_mods_button()
    _setup_cheats_button()

func OnModsPressed():
    ShowMods()

func ShowMods():
    print("[MODS] Opening mod menu...")
    Show("mods")
    ResetButtons()
    # Hide main menu elements
    var p = get_tree().current_scene.get_node_or_null("ui/ui parent main/ui parent_lobby home")
...
var _cheats_btn_ref: Control
var _cheats_label_ref: Control
...
func _process(_delta):
    ...
    if viewing_intro:
        ...
        if is_instance_valid(_cheats_btn_ref) and is_instance_valid(_options_btn_ref):
            _cheats_btn_ref.modulate.a = _options_btn_ref.modulate.a
        ...
        if is_instance_valid(_cheats_label_ref) and is_instance_valid(_options_label_ref):
            _cheats_label_ref.modulate.a = _options_label_ref.modulate.a
    else:
...
func Start():
    ...
    for w in waterfalls: if is_instance_valid(w): w.pause()
    speaker_start.play()
    cursor.SetCursor(false, false)
    savefile.ClearSave()
...
func _setup_cheats_button():
    var parent_container = get_tree().current_scene.get_node_or_null("ui/ui parent main/ui parent_lobby home")
    if not parent_container: return

    var options_btn = parent_container.get_node_or_null("sub options/button_sub options")
    var options_label = parent_container.get_node_or_null("sub options")
    var options_wrapper = _find_button_wrapper_recursive(options_btn)

    if not options_btn or not options_wrapper:
        print("[CHEATS] Could not find options button for positioning.")
        return

    var cheats_btn = options_btn.duplicate()
    var cheats_label = options_label.duplicate()
    parent_container.add_child(cheats_btn)
    parent_container.add_child(cheats_label)
    
    cheats_btn.name = "button_cheats"
    cheats_label.name = "cheats"
    
    # Find label inside
    var lbl = _find_label_recursive(cheats_label)
    if lbl: 
        lbl.text = "CHEATS"
        if lbl.has_method("set_text"): lbl.set_text("CHEATS")

    var gap = 45 # Default gap

    var menu_items = []
    for child in parent_container.get_children():
        var btn = child.get_node_or_null("button_" + child.name)
        if btn:
            menu_items.append({"wrapper": _find_button_wrapper_recursive(btn), "btn": btn, "lbl": child})

    var mods_btn = parent_container.get_node_or_null("button_mods")
    var mods_label = parent_container.get_node_or_null("mods")
    
    var insert_idx = -1
    for i in range(menu_items.size()):
        if menu_items[i].btn == mods_btn:
            insert_idx = i
            break
    ...
    for i in range(menu_items.size()):
        ...
        if i == insert_idx:
            cheats_btn.position = mods_btn.position
            cheats_btn.position.y = current_pos_y
            cheats_label.position = mods_label.position
            cheats_label.position.y = current_pos_y
            current_pos_y += gap

    cheats_btn.self_modulate.a = 0
    cheats_btn.mouse_filter = Control.MOUSE_FILTER_STOP
    cheats_label.mouse_filter = Control.MOUSE_FILTER_IGNORE

    _cheats_btn_ref = cheats_btn
    _cheats_label_ref = cheats_label

    var cheats_wrapper = _find_button_wrapper_recursive(cheats_btn)
    if cheats_wrapper:
        cheats_wrapper.ui = cheats_label
        cheats_wrapper.t = lbl
        cheats_wrapper.adding_cursor = true
        cheats_wrapper.alias = "cheats"
        cheats_wrapper.isActive = true
        cheats_wrapper.isDynamic = true
        cheats_wrapper.playing = true
        cheats_btn.modulate.a = 0

        for connection in cheats_wrapper.get_signal_connection_list("is_pressed"):
            cheats_wrapper.disconnect("is_pressed", connection["callable"])
        cheats_wrapper.connect("is_pressed", ShowCheats)

        if cheats_wrapper.t:
            cheats_wrapper.t.text = "CHEATS"
            cheats_wrapper.orig = "CHEATS"

        if not cheats_wrapper in buttons:
            buttons.append(cheats_wrapper)
        
        cheats_btn.modulate.a = 0
        cheats_label.modulate.a = 0

    cheats_btn.visible = true
    cheats_label.visible = true

    _setup_cheats_screen()

func ShowCheats():
    print("[CHEATS] Opening singleplayer cheat menu...")
    if is_instance_valid(parent_cheats):
        parent_cheats.visible = true
        # Hide other main menu elements similar to ShowMods
        var p = get_tree().current_scene.get_node_or_null("ui/ui parent main/ui parent_lobby home")
        if p: p.visible = false
        GlobalVariables.cursor_state_after_toggle = true
        # Set focus to something in cheats if needed

func _setup_cheats_screen():
    # Create a container for the cheat menu
    var ui_root = get_tree().current_scene.get_node_or_null("ui/ui parent main")
    if not ui_root: return
    
    parent_cheats = Control.new()
    parent_cheats.name = "ui parent_cheats"
    ui_root.add_child(parent_cheats)
    parent_cheats.visible = false
    parent_cheats.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
    
    # Add a background
    var bg = ColorRect.new()
    bg.color = Color(0, 0, 0, 0.8)
    parent_cheats.add_child(bg)
    bg.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
    
    # Add a header
    var header = Label.new()
    header.text = "SINGLEPLAYER CUSTOMIZATION"
    header.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
    parent_cheats.add_child(header)
    header.position = Vector2(0, 50)
    header.set_anchors_preset(Control.PRESET_TOP_WIDE)
    
    # VBox for settings
    var vbox = VBoxContainer.new()
    vbox.name = "settings_vbox"
    parent_cheats.add_child(vbox)
    vbox.set_anchors_and_offsets_preset(Control.PRESET_CENTER)
    vbox.custom_minimum_size = Vector2(400, 0)
    
    # Add back button
    var back_wrapper = Control.new()
    back_wrapper.name = "back_btn_wrapper"
    back_wrapper.custom_minimum_size = Vector2(0, 50)
    vbox.add_child(back_wrapper)
    
    var back_btn = Button.new()
    back_btn.text = "BACK"
    back_btn.name = "button_back"
    ...
    # Helper to add a row
    var add_row = func(label_text: String):
    ...
    # Starting Health
    var row_health = add_row.call("STARTING HEALTH:")
    var btn_health = Button.new()
    btn_health.text = str(sp_customization.starting_health)
    row_health.add_child(btn_health)
    btn_health.pressed.connect(func():
        sp_customization.starting_health += 1
        if sp_customization.starting_health > 6: sp_customization.starting_health = 1
        btn_health.text = str(sp_customization.starting_health)
        _apply_sp_cheats()
    )

    # Number of Rounds
    var row_rounds = add_row.call("NUMBER OF ROUNDS:")
    var btn_rounds = Button.new()
    btn_rounds.text = str(sp_customization.number_of_rounds)
    row_rounds.add_child(btn_rounds)
    btn_rounds.pressed.connect(func():
        sp_customization.number_of_rounds += 1
        if sp_customization.number_of_rounds > 3: sp_customization.number_of_rounds = 1
        btn_rounds.text = str(sp_customization.number_of_rounds)
        _apply_sp_cheats()
    )

    # Skip Intro
    var row_skip = add_row.call("SKIP INTRO:")
    var btn_skip = Button.new()
    btn_skip.text = "ON" if sp_customization.skipping_intro else "OFF"
    row_skip.add_child(btn_skip)
    btn_skip.pressed.connect(func():
        sp_customization.skipping_intro = !sp_customization.skipping_intro
        btn_skip.text = "ON" if sp_customization.skipping_intro else "OFF"
        _apply_sp_cheats()
    )

var sp_customization = {
    "starting_health": 4,
    "number_of_rounds": 3,
    "skipping_intro": false
}

func _apply_sp_cheats():
    # Update GlobalVariables
    GlobalVariables.skipping_intro = sp_customization.skipping_intro
    # We'll use a simplified version of the customization dictionary for SP
    GlobalVariables.active_match_customization_dictionary = {
        "number_of_rounds": sp_customization.number_of_rounds,
        "skipping_intro": sp_customization.skipping_intro,
        "starting_health_override": sp_customization.starting_health
    }
    print("[CHEATS] Applied SP cheats: ", GlobalVariables.active_match_customization_dictionary)

var parent_cheats: Control
...
func _setup_mods_utility_buttons():
    ...
    var refresh_node = mod_template_btn.duplicate()
    var refresh_label = mod_template_label.duplicate() if mod_template_label else null
    ...
    var mods_refresh_btn = _find_button_wrapper_recursive(refresh_node)
    ...
    setup_util.call(mods_restart_btn, restart_label, "EXIT GAME", "restart", func(): get_tree().quit())
    setup_util.call(mods_refresh_btn, refresh_label, "REFRESH MODS", "refresh", func(): 
        var ml = get_node_or_null("/root/ModLoader")
        if ml:
            ml.register_mods()
            RefreshModList()
    )
    setup_util.call(mods_folder_btn, folder_label, "OPEN MOD FOLDER", "open_folder", func(): 
        var path = ""
        if OS.has_feature("editor"):
            path = ProjectSettings.globalize_path("res://mods-unpacked")
        else:
            path = _ModLoaderPath.get_path_to_mods()
    ...
    add_row.call(refresh_node, refresh_label)
