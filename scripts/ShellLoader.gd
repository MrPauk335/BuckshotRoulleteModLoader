...
func LoadShells():
    ...
    if ( !roundManager.shellLoadingSpedUp): await get_tree().create_timer(0.8, false).timeout
    ...
    for i in range(numberOfShells):
        ...
        if (roundManager.shellLoadingSpedUp): await get_tree().create_timer(0.17, false).timeout
        else: await get_tree().create_timer(0.32, false).timeout
        pass
    animator_dealerHandRight.play("RESET")
    dealerAI.Speaker_HandCrack()
    if (roundManager.shellLoadingSpedUp): await get_tree().create_timer(0.17, false).timeout
    else: await get_tree().create_timer(0.42, false).timeout
    ...
    if roundManager.playerIsAI:
        roundManager.BeginAIPlayerTurn()
        return
